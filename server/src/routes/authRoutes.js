...
router.get("/profile", protect, (req, res) => {
    const { _id, name, email, role, provider, preferredProgrammingLanguage, practiceGoal, targetRole, weeklyPracticeTarget, reminderEnabled, reminderDay, reminderTime, reminderTimezone, plan, practiceSubscriptionStatus, isVerified } = req.user;
    res.json({ _id, name, email, role, provider, preferredProgrammingLanguage, practiceGoal, targetRole, weeklyPracticeTarget, reminderEnabled, reminderDay, reminderTime, reminderTimezone, plan, practiceSubscriptionStatus, isVerified });
});
...
router.get("/export", protect, requireFeature("ACCOUNT_DATA_EXPORT_ENABLED"), async (req, res, next) => {
    try {
        ...
        const [profile, interviews, resumes, resumeReviews, savedExperiences, productFeedback, reminderDeliveries, productEvents] = await Promise.all([
            User.findById(userId).select("name email role provider preferredProgrammingLanguage practiceGoal targetRole weeklyPracticeTarget reminderEnabled reminderDay reminderTime reminderTimezone plan practiceSubscriptionStatus isVerified createdAt updatedAt").lean(),
