...
export default async function bulkFeedbackProcessor(job) {
    const { roundId, items = [], attach = true } = job.data || {};
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const safeSuggestions = Array.isArray(gen?.suggestions)
            ...
            : [];
        const fb = await Feedback.create({ question: q._id, comment: safeComment, score: clampedScore, suggestions: safeSuggestions });
        createdCount++;
