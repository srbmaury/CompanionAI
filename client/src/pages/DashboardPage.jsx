...
const DashboardPage = () => {
    ...
    const inProgressInterview = useMemo(() => interviews.find((item) => !item.isCompleted) || null, [interviews]);
    const nextRecommendation = recommendations[0] || null;
    const secondaryRecommendations = inProgressInterview ? recommendations : recommendations.slice(1);
    const primaryAction = inProgressInterview
        ? { label: "Continue interview", href: `/practice/interviews/${inProgressInterview._id}`, icon: <PlayCircleOutline /> }
        : { label: "New interview", href: "/practice/new", icon: <Add /> };
        ...
        <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
            <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} spacing={2} mb={4}>
                <Box>
                    ...
                    <Typography color="text.secondary" mt={1}>{inProgressInterview ? "Pick up exactly where you left off, or review your progress before continuing." : "Keep your momentum going with focused, role-specific practice."}</Typography>
                </Box>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
                    ...
                    <Button variant="contained" size="large" startIcon={primaryAction.icon} onClick={() => navigate(primaryAction.href)} sx={{ flexShrink: 0 }}>{primaryAction.label}</Button>
                </Stack>
            </Stack>

            {!loading && inProgressInterview && <Card variant="outlined" sx={{ mb: 4, borderColor: "primary.light" }}>
                <CardActionArea onClick={() => navigate(`/practice/interviews/${inProgressInterview._id}`)}>
                    <CardContent sx={{ p: { xs: 2.5, md: 3.5 } }}>
                        <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ md: "center" }} gap={2.5}>
                            <Box sx={{ flex: 1 }}>
                                <Typography variant="overline" color="primary.main" fontWeight={850}>Continue where you left off</Typography>
                                <Typography component="h2" variant="h5" fontWeight={850}>{inProgressInterview.jobRole}</Typography>
                                <Typography color="text.secondary" mt={.5}>{inProgressInterview.company || "Role-specific practice"}{Number.isFinite(Number(inProgressInterview.roundsCompleted)) && Number.isFinite(Number(inProgressInterview.roundsTotal)) ? ` · ${inProgressInterview.roundsCompleted}/${inProgressInterview.roundsTotal} rounds complete` : ""}</Typography>
                                {Number(inProgressInterview.roundsTotal) > 0 && <LinearProgress variant="determinate" value={Math.min(100, (Number(inProgressInterview.roundsCompleted) / Number(inProgressInterview.roundsTotal)) * 100)} sx={{ height: 7, borderRadius: 99, mt: 2, maxWidth: 520 }} />}
                            </Box>
                            <Button variant="contained" endIcon={<ArrowForward />} sx={{ alignSelf: { xs: "stretch", md: "center" } }}>Continue interview</Button>
                        </Stack>
                    </CardContent>
                </CardActionArea>
            </Card>}

            {!loading && !inProgressInterview && Number(progress.completed || 0) > 0 && <Card variant="outlined" sx={{ mb: 4, borderColor: "primary.light" }}>
                <CardContent sx={{ p: { xs: 2.5, md: 3.5 } }}>
                    <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ md: "center" }} gap={2.5}>
                        <Box sx={{ flex: 1 }}>
                            <Typography variant="overline" color="primary.main" fontWeight={850}>Next best action</Typography>
                            <Typography component="h2" variant="h5" fontWeight={850}>{nextRecommendation?.title || "Run another focused interview"}</Typography>
                            <Typography color="text.secondary" mt={.5}>{nextRecommendation?.reason || "Use your latest feedback to practice the next high-signal area while it is still fresh."}</Typography>
                        </Box>
                        <Button variant="contained" endIcon={<ArrowForward />} onClick={() => navigate(canonicalProductPath(nextRecommendation?.href || "/practice/new"))} sx={{ alignSelf: { xs: "stretch", md: "center" } }}>Start next practice</Button>
                    </Stack>
                </CardContent>
            </Card>}
